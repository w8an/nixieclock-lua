...
time_zone = EST         -- Choose a time zone
hours12 = false         -- Set true for 12 hour display

inp_hour_sel = 34  -- switch to choose 12/24 hour display
inp_view_ip = 35      -- pushbutton to view IP address
...
mpx = function()  --run as a thread
  ...
  -- tube and digit output pins, lsd to msd
  -- tube ANODES A0,A1,etc.  i.e. tube_pos[1]=GPIO21
  tube_pos = {pio.GPIO21, pio.GPIO22, pio.GPIO23, pio.GPIO25, pio.GPIO26, pio.GPIO27} 
  -- binary to nixie decoder chip abcd inputs
...
upd_datetime = function() --run as a thread
  ...
  repeat
    now = os.time() + tz_offset   -- get epoch seconds
...
--    nixie_disp[i] = tonumber(disp_ara[i])
  end
end
